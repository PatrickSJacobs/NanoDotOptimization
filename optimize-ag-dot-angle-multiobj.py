...
def obj_func_run(x: [float]):
    ...
    file1.writelines(["#!/bin/bash%s" % "\n",
                      ...
                      "mpirun -np 4 meep no-metal?=true theta_deg=%s %s | tee %s%s" % (theta_deg, new_file, air_raw_path, "\n"),
                      #"meep no-metal?=true theta_deg=%s %s | tee %s%s" % (theta_deg, new_file, air_raw_path, "\n"),
                      "grep flux1: %s > %s%s" % (air_raw_path, air_data_path, "\n"),
                      "mpirun -np 4 meep sr=%s ht=%s sy=%s theta_deg=%s %s |tee %s;%s" % (sr, ht, cell_size, theta_deg, new_file, metal_raw_path, "\n"),
