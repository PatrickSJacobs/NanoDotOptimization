...
def sim(run_file, filenames=[], input_lines=[]):
    ...
    sleep(1)  # Pause to give time for simulation file to be created 
    ...
    commands = [
        ("Run simulation for air", f"mpirun -np 32 python -u {new_file} True | tee -a {air_raw_path} ; grep flux1: {air_raw_path} > {air_data_path}"),
        ("Run simulation for metal", f"mpirun -np 32 python -u {new_file} False | tee -a {metal_raw_path} ; grep flux1: {metal_raw_path} > {metal_data_path}"),
