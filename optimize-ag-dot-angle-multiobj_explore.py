...
def obj_func_run(x: [float]):
    ...
    file1.writelines(["#!/bin/bash%s" % "\n",
                      ...
                      "ibrun -np 64 meep no-metal?=true sy=%s theta_deg=%s %s | tee %s;%s" % (cell_size, theta_deg, new_file, air_raw_path, "\n"),
                      ...
                      "mpirun -np 64 meep no-metal?=false sr=%s ht=%s sy=%s theta_deg=%s %s |tee %s;%s" % (sr, ht, cell_size, theta_deg, new_file, metal_raw_path, "\n"),
...
problem = (
    ...
    #.add_variable(0.001 * 25, 0.001 * 250)
    .add_variable(0.001 * 350, 0.001 * 370)
    #.add_variable(0.0, 0.0)
    .add_variable(0.0, 0.0)
    .add_function(c)
    .add_function(b)
